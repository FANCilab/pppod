...
The plotting groups are hardcoded below from subject membership only.  Analysis
results are not expected to live inside group folders.
...
def plot_aggregate_vertical_center_density_summary(
    ...
    *,
    ...
    fig.suptitle(
        f"{group}: aggregate good-RF center maps and Gaussian densities | "
        f"EV > {min_ev:g}, p < {max_p_value:g}, good P/N > {min_peak_to_noise:g}",
        fontweight="bold",
        y=0.985,
    )
    fig.subplots_adjust(left=0.12, right=0.98, bottom=0.055, top=0.94, hspace=0.24)
    save_figure(fig, output_dir / f"{group}_aggregate_good_RF_center_density_vertical_summary", dpi=300)
    return 1
