...
i = i+1;
db(i).subject    = 'NM039'; % animal name
db(i).date          = '20260610'; % date of the recording
db(i).exp         = [2]; % all the experiments in the recording
db(i).expID         = 1; % the experiment you want to compute pixel map of
db(i).n_planes       = 2;
db(i).fun_channel   = 1;
...
results = analyze_grating_experiment(data, targetFolder, opts);
    ...
    %%
% %%OLD CODE, CONTAINS USEFUL THINGS TO IMPLEMENT LIKE TUNING CURVES
% 
