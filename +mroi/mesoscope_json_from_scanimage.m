...
% try
[stack, hh] = tiff.loadFramesBuff(fname,1,1,1);
% hh = hh{1};
...
try
vars = getVarFromHeader(hSIh, ...
            {'\nSI.hRoiManager.', '\nSI.hStackManager.'},...
            {'scanVolumeRate', 'zs'});

fs = str2double(vars{1});
zs = sscanf(vars{2}(2:end-1), '%f');
nplanes = numel(zs);

catch
    fs = 5.869041182442207;
    zs = 0;
    nplanes = 1;
end
%%
...
irow(end) = [];
irow(2,:) = irow(1,:) + Ly'; % I think you need to subtract 1 to the first 
